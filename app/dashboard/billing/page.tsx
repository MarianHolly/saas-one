...
const featureItems = [
  { name: "Large Storage Capacity" },
  { name: "Advanced Organization Tools" },
  { name: "Priority Customer Support" },
  { name: "Cross-Device Syncing" },
  { name: "Exclusive Features and Updates" },
];
...
async function getData(userId: string) {
  ...
  const data = await prisma.subscription.findUnique({
    where: { userId: userId },
    select: { status: true, user: { select: { stripeCustomerId: true } } },
  });
      ...
      <Card className="flex flex-col">
        <CardContent className="py-8">
          ...
          <p className="mt-5 text-lg text-muted-foreground">
            Write as many notes as you want for 15€ a Month
          </p>
        </CardContent>
